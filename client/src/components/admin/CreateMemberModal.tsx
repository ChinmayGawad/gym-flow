...
export const CreateMemberModal: React.FC<CreateMemberModalProps> = ({
  ...
  const handleCreateMember = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Create member via auth client admin API
      ...
      // 2. Assign plan in database if created
      const userId = res?.data?.user?.id;
      if (userId && plan !== 'basic') {
        try {
          await fetch(`${API_BASE}/api/admin/members/${userId}/plan`, {
            method: 'PATCH',
          ...
          // Non-blocking plan sync
        }
      }

      setSuccessMessage(`Member "${name}" successfully registered!`);
      setTimeout(() => {
        resetForm();
        onClose();
      }, 900);
      ...
      <DialogContent className="sm:max-w-[480px] p-6 max-h-[90vh] overflow-y-auto bg-white dark:bg-[#131418] border border-black/[0.08] dark:border-white/[0.08] rounded-2xl shadow-2xl">
        ...
        <form onSubmit={handleCreateMember} className="space-y-4 mt-3">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="member-name" className="text-xs font-bold text-zinc-700 dark:text-zinc-300">Member Full Name</Label>
          ...
          {/* Email Address */}
          <div className="space-y-1.5">
            <Label htmlFor="member-email" className="text-xs font-bold text-zinc-700 dark:text-zinc-300">Email Address</Label>
            <div className="relative">
          ...
          {/* Initial Password */}
          <div className="space-y-1.5">
            <Label htmlFor="member-password" className="text-xs font-bold text-zinc-700 dark:text-zinc-300">Initial Password</Label>
            <div className="relative">
