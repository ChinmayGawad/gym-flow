...
export const TodayRushSparkline: React.FC<TodayRushSparklineProps> = ({ capacity = 30 }) => {
  ...
  return (
    <Card className="p-5 sm:p-6 bg-white border border-black/[0.06] shadow-card">
      {/* Header Info */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pb-4 border-b border-zinc-100">
        ...
        <Link
          to="/schedule"
          className="text-xs font-bold text-zinc-600 hover:text-zinc-950 flex items-center gap-1 transition-colors self-start sm:self-center group"
        >
      ...
      <div className="pt-5 pb-2">
        <div className="flex items-end justify-between gap-1.5 sm:gap-2 h-24 sm:h-28 w-full relative">
          {HOURLY_FACTORS.map((item, idx) => {
            ...
            // Bar background color based on status
            ...
            return (
              ...
              >
                {/* Tooltip on Hover */}
                {isHovered && (
                  <div className="absolute -top-10 left-1/2 -translate-x-1/2 z-20 pointer-events-none bg-zinc-900 text-white text-[10px] font-bold px-2 py-1 rounded-lg shadow-card border border-zinc-800 whitespace-nowrap flex items-center gap-1.5">
                    <span>{item.time}:</span>
                    <span className="text-zinc-200">~{people} people</span>
                    <span
                ...
                <div
                  className={`w-full max-w-[18px] sm:max-w-[22px] rounded-t-md transition-all duration-200 ${barBg} ${
                    isNow ? 'ring-2 ring-zinc-900 ring-offset-1' : ''
                  }`}
                ...
                {isNow && (
                  <div
                    className="w-1.5 h-1.5 rounded-full bg-zinc-900 mt-1"
                    title="Current Hour"
                  />
                )}
              </div>
        ...
        <div className="flex justify-between text-[10px] font-medium text-zinc-400 mt-2 px-1 tabular-nums">
          <span>6 AM</span>
          <span>9 AM</span>
          <span className="text-emerald-700 font-bold">11 AM (Quiet)</span>
          <span>2 PM</span>
          <span className="text-rose-600 font-bold">6 PM (Rush)</span>
          <span>9 PM</span>
          <span>10 PM</span>
        </div>
      </div>
